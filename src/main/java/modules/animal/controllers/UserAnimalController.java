...
public class UserAnimalController {
    ...
    private void loadData() {
        ...
        if (userId <= 0) return; // Si non connecté
        try {
            List<Animal> list = service.getByUserId(userId);
            animalList = FXCollections.observableArrayList(list);
            tableAnimal.setItems(animalList);
        } catch (SQLException e) {
            AlertUtils.showError("Erreur SQL", "Impossible de charger la liste de vos animaux: " + e.getMessage());
        }
    }
    ...
    void ajouterAnimal() {
        ...
        int userId = SessionManager.getCurrentUserId();
        if (userId <= 0) {
            AlertUtils.showError("Erreur", "Vous devez être connecté.");
            return;
        }
